...
The most frequently used improvement functions are \emph{Best Improvement}(or steepest descent) heuristic (cf. Algorithm~\ref{bestimprov}), which exhaustively looks at each neighbor to select the best solution, and \emph{First Improvement} (or first descent) heuristic (cf. Algorithm~\ref{firstimprov}), which returns as soon as a better neighbor is found. 
...
\If {\Call{$cost$}{$x_i$} $\leq$ \Call{$cost$}{$x$}}
    \State $x \gets x_i$
\EndIf
\Until{$i=|N(x)|$}
\EndFunction
...
\State $i \gets i+1$
\State $x_i \in N(x)$, retrieve the i-th neighbor
\Until{$i=|N(x)|$ or \Call{$cost$}{$x_i$} $\leq$ \Call{$cost$}{$x$}}
\EndFunction
\end{algorithmic}
\end{algorithm}
