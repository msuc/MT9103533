...
The simple local search (LS) (cf. Algorithm~\ref{localsearchalgo}) finds a local optimum according to a cost function $f_c(x)$ in a region around an initial solution in the solution space, defined by a fixed neighborhood structure $N(x)$. 
...
Another successful Meta-Heuristic strategy is Tabu Search (cf. Algorithm~\ref{tabualgo}) (see \cite{glover1988tabu}). 
The main contribution is the use of memory structures which are used to guide the local search procedure. The memory lists keeps track of moves which are not allowed to be revisited for a given amount of time during the exploration of the search space - they are \emph{tabu}\footnote{The word tabu (or taboo) stems from an Polynesian language and indicates things which may not be touched.(see~\cite{glover1997tabu})} 
In its simplest form the Tabu list includes complete visited solutions. 
