...
To solve hard problems due to large solution space, hard run time constraints, or both, finding an exact global optimal solution is not always feasible. Instead a \emph{good} approximate solution, with the help of an heuristic is used and optimality is traded off with good performance.
...
The basic local search (BLS) (cf. Algorithm~\ref{localsearchalgo}) finds a local optimum according to a cost function $cost(x)$ and a fixed sized region around an initial solution in the solution space. 
...
A very different approach in guiding the search for optimal solutions than the previous introduced methods is Guided Local Search (GLS) which was introduced in TODO: cite [Voudouris and Tsang 1999]. While the aforementioned algorithms like ILS and VNS change the neighborhood of solutions, this approach changes the objective function itself to achieve the desired diversification. The neighborhood structure stays fixed. In Figure~\ref{fig:fig_gls} the gradually change of the cost function which may lead to new local minimal solutions is visualized. 
\begin{figure}[thb] 
   \footnotesize
   \centering
    \def\svgwidth{0.75\textwidth}
